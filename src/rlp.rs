...
mod Rlp {
    ...
    use self::RlpEncodable::{Item, List};
    ...
    impl RlpEncodable {
        pub fn encode(self) -> RlpDecodable {
            RlpDecodable(
                ...
                match self { 
                    Item(v) => {
                        if v.len() == 1 && v[0] < ITEM_OFFSET { v }
                        else { RlpEncodable::encode_next_length(v.len(), ITEM_OFFSET)} 
                    },
                    ...
                    List(v) => {
                        ...
                        RlpEncodable::encode_next_length(data)
                    }
                }
            )
        }
        ...
        fn encode_next_length(length: uint, offset: u8) -> Vec<u8> {
            ...
            for length_of_length in range(0, 8) {
                if length < u32.pow(length_of_length + 1) {
    ...
    impl RlpDecodable {
        ...
        fn decode_with_bufreader(reader: &mut BufReader) -> RlpEncodable {
            match reader.read_byte() {
                Ok(byte) if byte < ITEM_OFFSET => {
                    Item(vec![byte])
                },

                Ok(byte) if byte < LIST_OFFSET => {
                    let length = RlpDecodable::decode_next_length(reader, byte, ITEM_OFFSET);
                    Item(reader.read_exact(length).unwrap())
                },
...
mod tests {
    ...
    use super::RlpEncodable::{Item, List};

    macro_rules! s(($s:expr) => (Item(String::from_str($s).into_bytes())));
